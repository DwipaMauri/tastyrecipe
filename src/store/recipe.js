...
export default {
  ...
  actions: {
    async getRecipeData({ commit }) {
      try {
        const { data } = await axios.get(
          "https://tasty-recipe-52ebd-default-rtdb.firebaseio.com/recipes.json"
        );
        const arr = [];
        for (let key in data) {
    ...
    async getRecipeDetail({ commit }, payload) {
      try {
        const { data } = await axios.get(
          `https://tasty-recipe-52ebd-default-rtdb.firebaseio.com/recipes/${payload}.json`
        );
    ...
    async addNewRecipe({ commit, rootState }, payload) {
      ...
      try {
        const { data } = await axios.post(
          `https://tasty-recipe-52ebd-default-rtdb.firebaseio.com/recipes.json?auth=${rootState.auth.token}`,
          newData
        );
    ...
    async deleteRecipe({ dispatch, rootState }, payload) {
      try {
        const { data } = await axios.delete(
          `https://tasty-recipe-52ebd-default-rtdb.firebaseio.com/recipes/${payload}.json?auth=${rootState.auth.token}`
        );
    ...
    async updateRecipe({ dispatch, rootState }, { id, newRecipe }) {
      try {
        const { data } = await axios.put(
          `https://tasty-recipe-52ebd-default-rtdb.firebaseio.com/recipes/${id}.json?auth=${rootState.auth.token}`, newRecipe
        );

        await dispatch("getRecipeData");
        // commit("setNewRecipe", data);
      } catch (err) {
        console.log(error);
      }
    },
